...
* Un utilisateur non connecté arrive sur `/login` et se connecte avec
  Microsoft.
* En développement local avec `AUTH_MODE=dev`, `/dev-login` permet de choisir
  une fixture allowlistée sans appeler Microsoft et affiche un bandeau visible.
...
* Le mode `dev` est refusé hors de `NODE_ENV=development` et hors loopback ; il
  crée une session Express normale avec les mêmes permissions et le même CSRF.
...
* `GET /api/auth/dev-fixtures`, `POST /api/auth/dev-login` : fixtures locales,
  uniquement lorsque `AUTH_MODE=dev` est autorisé.
* `GET /api/auth/me` : identité, `baseRole`, rôle effectif, `entityId`,
  `status`, incarnation active, `csrfToken`, `authMode`.
...
* `frontend/src/pages/dev-login.page.tsx`
