...
* Review pilote (jalon 1) : `docs/history/reviews/2026-07-31-authentification-microsoft-entra-pilot.md`
...
* `docs/operations/deployment.md` : nouvelle section "Secrets authentification
...
* `backend/.env.example` mis à jour (renvoie vers `/etc/gnu-gesta/backend.env`
  en production).
* `docs/current/architecture.md`, `docs/current/data-model.md`, `docs/current/features.md` : section
...
* Déploiement : `deploy/systemd/gnu-gesta-backend.service`,
  `backend/.env.example`, `docs/operations/deployment.md`.
* Documentation : `docs/current/architecture.md`, `docs/current/data-model.md`,
  `docs/current/features.md`, README des features `auth`/`students`/`companies`/`offers`,
  ce document, `docs/history/phases/2026-07-31-authentification-microsoft-entra-v1-plan.md`
...
* **Matrice `GET /api/students`/`GET /api/companies` élargie** à toute
  ...
  (voir "Écarts"). Cohérent avec `docs/current/architecture.md` ("les lecteurs sont
...
* **`isProductionLikeEnvironment()` traite `staging` comme `production`**
  ...
  `docs/operations/deployment.md` — pas seulement `NODE_ENV=production` littéralement.
...
* Créer `/etc/gnu-gesta/backend.env` sur le VPS (permissions `600`) avec les
  ...
  à la production (voir `docs/operations/deployment.md`).
